...
export class PreviewGraph implements OnInit {
      ...
      if(event.target.checked == true){
        if(this.data.length > 1){
          ...
          while(i < aux.length){
            j = i + 1;
            while(j < aux.length){
              var aux1 = aux[i][this.propertyLabelSelected];
              var aux2 = aux[j][this.propertyLabelSelected];
  ...
  updateGraph(){
    ...
    var aux = this.getDataArray(this.propertyLabelSelected); 
