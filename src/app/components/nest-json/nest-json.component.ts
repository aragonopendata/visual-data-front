...
export class NestJSONComponent {
    ...
    actionMapping: {
      mouse: {
        drop: (tree: TreeModel, node: TreeNode, $event: any, { from, to }) => {
          this.nodes1 = [];
          const clone = JSON.parse(JSON.stringify(from.data));
  ...
  constructor() {
    ...
    const data = {
      ...
      city: {
        asd: 'data',
        rdf: { asd: 'data', cabezera: ['Arturo', 'Jaime', 'Mario'] }
      }
    };
    // [31,25,15]
    // { asd: "data", rdf : "cosa"}
    const n2 = new Nested('dato', []);

    const n = new Nested('dato', [n2]);
  ...
  recursiveNest(data): Array<Nested> {
    if (typeof data === 'object') {
      ...
      keys.forEach(element => {
        const array2 = this.recursiveNest(data[element]);
  ...
  arrayDataOfNest(data): Array<string> {
    ...
    try {
      ...
      console.log(
        'El dato proporcionado para esta functión no es una Array de datos'
      );
    }
    return aux;
  }
