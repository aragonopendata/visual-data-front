...
export class DatasetsService {
    ...
    public getDatasets(sort: string, page: number, rows: number, type: string) {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS
    ...
    public getDatasetHomerByPackageId(datasetHomerName: string) {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_HOMER
            + '/' + datasetHomerName;
        return this.http.get(fullUrl).map(res => res.json());
    }
    public getDatasetsByTopic(topicName: string, sort: string, page: number, rows: number, type: string) {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_TOPIC
            + '/' + topicName
            + '?' + Constants.SERVER_API_LINK_PARAM_SORT + '=' + sort
            + '&' + Constants.SERVER_API_LINK_PARAM_PAGE + '=' + page.toString()
            + '&' + Constants.SERVER_API_LINK_PARAM_ROWS + '=' + rows.toString()
    ...
    public getDatasetsByOrganization(organizationName: string, sort: string, page: number, rows: number, type: string) {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_ORGANIZATION
            + '/' + organizationName
            + '?' + Constants.SERVER_API_LINK_PARAM_SORT + '=' + sort
            + '&' + Constants.SERVER_API_LINK_PARAM_PAGE + '=' + page.toString()
            + '&' + Constants.SERVER_API_LINK_PARAM_ROWS + '=' + rows.toString()
    ...
    public getDatasetsBytags(sort: string, page: number, rows: number, tags: string[]) {
        ...
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_TAGS
            + '?' + Constants.SERVER_API_LINK_PARAM_SORT + '=' + sort
            + '&' + Constants.SERVER_API_LINK_PARAM_PAGE + '=' + page.toString()
            + '&' + Constants.SERVER_API_LINK_PARAM_ROWS + '=' + rows.toString()
    ...
    public getDatasetsByStats(sort: string, page: number, rows: number, groupName: string) {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_STATS_SEARCH
            + '/' + groupName
            + '?' + Constants.SERVER_API_LINK_PARAM_SORT + '=' + sort
            + '&' + Constants.SERVER_API_LINK_PARAM_PAGE + '=' + page.toString()
            + '&' + Constants.SERVER_API_LINK_PARAM_ROWS + '=' + rows.toString();
    ...
    public getDatasetResourceView(resoruce_id: string) {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_URL_DATASETS_RESOURCE_VIEW
            + '?' + Constants.SERVER_API_LINK_PARAM_RESOURCE_ID + '=' + resoruce_id;
        return this.http.get(fullUrl).map(res => res.json());
    }
    public getNewestDataset() {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_NEWEST;
        return this.http.get(fullUrl).map(res => res.json());
    }
    public getDownloadedDataset() {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_DOWNLOADED;
        return this.http.get(fullUrl).map(res => res.json());
    }
    public getDatasetsNumber() {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_COUNT;
        return this.http.get(fullUrl).map(res => res.json());
    }
    public getResourcesNumber() {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_RESOURCES_COUNT;
        return this.http.get(fullUrl).map(res => res.json());
    }
    public getDatasetsAutocomplete(text: string, limit: number) {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_AUTOCOMPLETE
            + '?' + Constants.SERVER_API_LINK_PARAM_TEXT + '=' + text
    ...
    public getDatasetRDF(datasetName: string) {
        const fullUrl = Constants.AOD_API_WEB_BASE_URL + Constants.SERVER_API_LINK_DATASETS_RDF
            + '/' + datasetName;
        return this.http.get(fullUrl).map(res => res.json());
    }
